...
$prewiv = htmlspecialchars_decode($arItem["full-text"]);
$prewiv = strip_tags($prewiv, '<a> <img>');
$prewiv = $obParser->html_cut($prewiv, 400);

$descr = strip_tags($arItem["description"], '<a> <img>');

$arItem["full-text"] = htmlspecialchars_decode($arItem["full-text"]);

$arItem["full-text"] = strip_tags($arItem["full-text"], '<a> <img> <p>');

 
//echo "<pre>"; print_r($prewiv); echo "</pre>";
?>

<item turbo="true">
	<title><?=$arItem["title"]?></title>
	<link><?=$arItem["link"]?></link>
	<turbo:content><? if($arItem["description"]){ 
		echo '<p>'.$descr.'</p>';
		}else{
			echo '<p>'.$prewiv.'</p>';
			}?>
