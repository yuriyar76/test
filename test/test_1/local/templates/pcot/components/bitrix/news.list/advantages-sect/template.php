...
?>
<div class="preim-box">
  <div id="content-preim" class="owl-carousel owl-theme" >
    <?foreach($arResult['ITEMS'] as $items):?>
    <div class="item-preim"><img src="<?=$items['DETAIL_PICTURE']['SRC']?>" data-toggle="tooltip" data-placement="bottom" title="<?=$items['DETAIL_TEXT']?>" alt="<?=$items['PREVIEW_TEXT']?>"><a data-toggle="tooltip" data-placement="bottom" title="<?=$items['DETAIL_TEXT']?>" class="preim-hover"></a></div>
    <?endforeach;?>
  </div>
</div>
...
<div class="row section-title d-flex flex-row justify-content-center">
    <div class="title">
        <h2 class="title-h1"><?=$arResult['SECTION']['PATH'][0]['NAME']?></h2>
    </div>
</div>
<?foreach($arResult['ITEMS'] as $items):?>
<div class="col-md-12 about-wrap_wrap d-flex flex-row flex-wrap justify-content-around">
                <div class="advantages_content_wrap d-flex flex-row justify-content-between">
                        ...
                        <div class="advantages_content d-flex flex-column justify-content-center">
                            <h3><?if(!empty($items['DISPLAY_PROPERTIES'])){?>
                                <a class="modal-link" href="#" title="<?=$items['PROPERTIES']['TITLE_MODAL']['VALUE'];?>" data-toggle="modal" data-target="#modal-<?=$items['PROPERTIES']['ID_MODAL']['VALUE'];?>"><?}?><?=$items['NAME'];?><?if(!empty($items['DISPLAY_PROPERTIES'])){?></a><?}?></h3>
